...
<a href="/enfermedades/{{$system->id}}" class="breadcrumb">Sistema {{$system->name}}</a>
...
<form action="/enfermedad/{{$diseases->id}}" method="POST" enctype="multipart/form-data">
	{{ csrf_field() }}
		<div>
			...
			<div class="col s12">
				<input type="hidden" name="symptoms" value="@foreach($chips as $chip) {{ $chip }}, @endforeach" id ="symptoms">
